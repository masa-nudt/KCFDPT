...
private:
  ...
  void punishBGEdges( arrayf &E );  //punish background edges
};
...
{
  clusterEdges(E,O,V); prepDataStructs(E); punishBGEdges(E); scoreAllBoxes(boxes);
}
...
void EdgeBoxGenerator::clusterEdges( arrayf &E, arrayf &O, arrayf &V )
{
  ...
  for( c=0; c<w; c++ ) for( r=0; r<h; r++ ) {
    if( c==0 || r==0 || c==w-1 || r==h-1 || E.val(c,r)<=_edgeMinMag ) //ignore edges on img boundary
  ...
  for( c=1; c<w-1; c++ ) for( r=1; r<h-1; r++ ) {
    ...
    while( sumv < _edgeMergeThr ) {
      _segIds.val(c0,r0)=_segCnt;                                     //_segIds contains the mask of edge group id 
  ...
  _segMag.resize(_segCnt,0);                                //size = edge group num, contains the total edge response of group
  for( c=1; c<w-1; c++ ) for( r=1; r<h-1; r++ )
  ...
  i=1; while(i>0) {
    i=0; for( c=1; c<w-1; c++ ) for( r=1; r<h-1; r++ ) {
      ...
      _segIds.val(c,r)=j; if(j>0) i++;                    //_segMag now ignores the response of post-merged edges    
    }
  }
  ...
  for( c=1; c<w-1; c++ ) for( r=1; r<h-1; r++ )
    if( (j=_segIds.val(c,r))>0 ) _segIds.val(c,r)=map[j];     //remove the removed edge groups from _segIds
  ...
  for( c=1; c<w-1; c++ ) for( r=1; r<h-1; r++ ) {
    ...
    float m=E.val(c,r), o=O.val(c,r); _segMag[j]+=m;         //_segMag now considers the response of post-merged edges
    meanOx[j]+=m*cos(2*o); meanOy[j]+=m*sin(2*o);
    meanX[j]+=m*c; meanY[j]+=m*r;                            //edge response acts as the weight for computing averages 
  }
  ...
  for( c=1; c<w-1; c++ ) for( r=1; r<h-1; r++ )
    if( (j=_segIds.val(c,r))>0 ) { _segC[j]=c; _segR[j]=r; }    //random draw a position for each edge group
...
void EdgeBoxGenerator::prepDataStructs( arrayf &E )
{
  ...
  for( i=0; i<10000; i++ )
    _scaleNorm[i]=pow(1.f/i,_kappa);       //create a list for division and pow

  // create _segIImg  -->  moved to punishBGEdges
...
//  }
  ...
  // create _magIImg  -->  moved to punishBGEdges
//  _magIImg.init(h+1,w+1);
//  for( c=1; c<w; c++ ) for( r=1; r<h; r++ ) {
//    float e = E.val(c,r) > _edgeMinMag ? E.val(c,r) : 0;    //how about removed edge groups?
...
void EdgeBoxGenerator::punishBGEdges( arrayf &E )
{
  //we can't punish background edges in a very small window
  int c, r, i;
  if( w<=_backSup_size || h<=_backSup_size )
  {
	  //create _segIImg
	  arrayf E1; E1.init(h,w);
	  for( i=0; i<_segCnt; i++ ) if( _segMag[i]>0 ) {
  ...
  //compute the affinity weight to background
  float *sWts=_sWts._x; int sId=_sId++;
  int *sDone=_sDone._x, *sMap=_sMap._x, *sIds=_sIds._x;
  int j, k, q, r0, c0, r1, c1;
  int cs, ce, rs, re, n=0;
  //the forground bounding box. any edge outside or across it is considered as background edge
  r1=h-2; r0=1;
  c1=w-2; c0=1;
  cs=_hIdxImg.val(c0,r0); ce=_hIdxImg.val(c1,r0); // top
	...
	vectorf segPunish;
	float p;
	segPunish.resize(_segCnt,0);

	//punish _segMag
	for( i=0; i<n; i++ ) {
	  k = sIds[i];
	  //if( _segC[k]>=c0 && _segC[k]<=c1 && _segR[k]>=r0 && _segR[k]<=r1 )
		// v -= sWts[i]*_segMag[k];  --> punish
	  p=(sWts[i]/4);
	  segPunish[k]=p;
	  _segMag[k]=_segMag[k]*(1-p);
	}

  //punish E
  for( c=1; c<w-1; c++ ) for( r=1; r<h-1; r++ ) {
	j=_segIds.val(c,r); if(j<=0) continue;
    if(segPunish[j]>0) E.val(c,r)=E.val(c,r)*(1-segPunish[j]);
  }
...
void EdgeBoxGenerator::scoreBox( Box &box )
{
  ...
  float *sWts=_sWts._x; int sId=_sId++;
  int *sDone=_sDone._x, *sMap=_sMap._x, *sIds=_sIds._x;   //bb: r, c, h, w, s
  // add edge count inside box
  r1=clamp(box.r+box.h,0,h-1); r0=box.r=clamp(box.r,0,h-1);
  c1=clamp(box.c+box.w,0,w-1); c0=box.c=clamp(box.c,0,w-1);    //bb: c0,r0  ->  c1,r1
...
void EdgeBoxGenerator::scoreAllBoxes( Boxes &boxes )
{
  ...
  scNum = int(ceil(log(max(w,h)/minSize)/log(_scStep)));         //scale num
  for( int s=0; s<scNum; s++ ) {
    ...
    for( a=0; a<2*arRad+1; a++ ) {
      ...
      for( c=0; c<w-bw+kc; c+=kc ) for( r=0; r<h-bh+kr; r+=kr ) {
        Box b; b.r=r; b.c=c; b.h=bh; b.w=bw; boxes.push_back(b);   //add boxes with fixed scale and aspect ratio
      }
    }
  }
  ...
  for( i=0; i<m; i++ ) {
    scoreBox(boxes[i]);
    if( !boxes[i].s ) continue; k++;    //non-refined boxes won't be considered
    refineBox(boxes[i]);
  }
  sort(boxes.rbegin(),boxes.rend(),boxesCompare);    //higher score first
...
//  int w = (int) mxGetN(pr[0]); O._w=E._w=w;
  ...
  float *eP = new float[h*w];
  memcpy(eP,(float*) mxGetData(pr[0]),w*h*sizeof(float));
  ...
  edgeBoxGen._backSup_size = float(mxGetScalar(pr[13]));
